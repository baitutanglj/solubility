...
def metric_classification(predict_df, args):
    ...
    performance_df.loc[0,'recall'] = round(recall_score(predict_df['target_label'], predict_df['y_pred_int'], average=average_type), 5)
    #精确率
    performance_df.loc[0, 'precision'] = round(precision_score(predict_df['target_label'], predict_df['y_pred_int'], average=average_type), 5)
    #F1-score
    performance_df.loc[0, 'f1'] = round(f1_score(predict_df['target_label'], predict_df['y_pred_int'], average=average_type), 5)
    # Kappa系数
